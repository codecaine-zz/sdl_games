...
pub fn render_legend_of_kage_game(renderer &sdl.Renderer, mut g LegendOfKageGame) {
...
fn render_scrolls(renderer &sdl.Renderer, mut g LegendOfKageGame, ox int, oy int) {
	for sc in g.scrolls {
		if !sc.active {
			continue
		}
...
fn render_projectiles(renderer &sdl.Renderer, mut g LegendOfKageGame, ox int, oy int) {
...
fn render_enemies(renderer &sdl.Renderer, mut g LegendOfKageGame, ox int, oy int) {
	for e in g.enemies {
		if !e.active {
			continue
		}
...
fn render_kage_player(renderer &sdl.Renderer, mut g LegendOfKageGame, ox int, oy int) {
	p := g.player
	px := int(p.x) + ox
	py := int(p.y) + oy
	pw := int(p.width)
