...
// Procedural Fever & Chill BGM Theme synthesizers
...
pub fn (sm &SoundManager) play_match() {
	if !sm.sound_enabled || sm.dev == 0 {
		return
	}
	sample_rate := 44100
	...
	for i in 0 .. num_samples {
		t := f64(i) / f64(sample_rate)
		s1 := math.sin(2.0 * math.pi * 880.0 * t) // A5
		s2 := math.sin(2.0 * math.pi * 1318.5 * t) // E6
		env := math.exp(-14.0 * t)
...
pub fn (sm &SoundManager) play_combo() {
	if !sm.sound_enabled || sm.dev == 0 {
		return
	}
	sample_rate := 44100
...
pub fn (sm &SoundManager) play_hard_drop() {
	...
	for i in 0 .. num_samples {
		t := f64(i) / f64(sample_rate)
		progress := f64(i) / f64(num_samples)
		freq := 220.0 - 140.0 * progress
		sq := if math.sin(2.0 * math.pi * freq * t) > 0.0 { 1.0 } else { -1.0 }
		env := math.exp(-12.0 * progress)
		pcm[i] = i16(sq * env * 22000.0)
	}
	sdl.queue_audio(sm.dev, pcm.data, u32(num_samples * 2))
}
