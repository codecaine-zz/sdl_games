...
fn render_hold_tray(renderer &sdl.Renderer, mut g DrMarioGame) {
	...
	if g.has_hold_pill {
		...
		render_pill_half(renderer, hx + 6, hy + 12, g.hold_c1, .left)
		render_pill_half(renderer, hx + 30, hy + 12, g.hold_c2, .right)
	}
}
	...
	for r in 0 .. 16 {
		for c in 0 .. 8 {
			...
			if cell.cell_type == .virus {
				render_virus_cell(renderer, cell_x, cell_y, cell.color, g.anim_timer)
			} else {
				...
				render_pill_half(renderer, cell_x, cell_y, cell.color, half_type)
			}
		}
	}
}
...
fn render_pill_half(renderer &sdl.Renderer, x int, y int, color PillColor, half_type PillHalfType) {
...
fn render_virus_cell(renderer &sdl.Renderer, x int, y int, color PillColor, anim_timer f32) {
...
fn render_active_pill(renderer &sdl.Renderer, mut g DrMarioGame, ox int, oy int) {
	...
	match p.orientation {
		.horizontal {
			render_pill_half(renderer, bx + p.x * 24, by + p.y * 24, p.c1, .left)
			render_pill_half(renderer, bx + (p.x + 1) * 24, by + p.y * 24, p.c2, .right)
		}
		.vertical {
			render_pill_half(renderer, bx + p.x * 24, by + p.y * 24, p.c1, .bottom)
			if p.y > 0 {
				render_pill_half(renderer, bx + p.x * 24, by + (p.y - 1) * 24, p.c2, .top)
			}
		}
	}
}
...
fn render_microscope_dish(renderer &sdl.Renderer, mut g DrMarioGame) {
	...
	if g.red_viruses > 0 {
		...
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: rx, y: ry, w: 36, h: 32 })
		// Dancing Arms
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: rx - 6, y: ry + 10 + arm_w, w: 6, h: 10 })
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: rx + 36, y: ry + 10 - arm_w, w: 6, h: 10 })
		// Big angry eyes
		sdl.set_render_draw_color(renderer, 255, 255, 255, 255)
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: rx + 6, y: ry + 6, w: 8, h: 10 })
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: rx + 22, y: ry + 6, w: 8, h: 10 })
		sdl.set_render_draw_color(renderer, 10, 10, 10, 255)
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: rx + 8, y: ry + 10, w: 4, h: 4 })
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: rx + 24, y: ry + 10, w: 4, h: 4 })
	}
	...
	if g.yellow_viruses > 0 {
		...
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: yx, y: yy, w: 34, h: 28 })
		// Smiling Eyes
		sdl.set_render_draw_color(renderer, 255, 255, 255, 255)
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: yx + 4, y: yy + 5, w: 8, h: 8 })
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: yx + 20, y: yy + 5, w: 8, h: 8 })
		sdl.set_render_draw_color(renderer, 10, 10, 10, 255)
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: yx + 6, y: yy + 7, w: 4, h: 4 })
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: yx + 22, y: yy + 7, w: 4, h: 4 })
	}
	...
	if g.blue_viruses > 0 {
		...
		by := my + 8 + wiggle
		sdl.set_render_draw_color(renderer, 45, 140, 245, 255)
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: bx, y: by, w: 32, h: 30 })
		// Wide Eyes
		sdl.set_render_draw_color(renderer, 255, 255, 255, 255)
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: bx + 4, y: by + 6, w: 8, h: 8 })
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: bx + 18, y: by + 6, w: 8, h: 8 })
		sdl.set_render_draw_color(renderer, 10, 10, 10, 255)
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: bx + 6, y: by + 8, w: 4, h: 4 })
		sdl.render_fill_rect(renderer, &sdl.Rect{ x: bx + 20, y: by + 8, w: 4, h: 4 })
	}
}
...
fn render_dr_mario_mascot(renderer &sdl.Renderer, mut g DrMarioGame) {
	...
	// Next Pill in Hand
	render_pill_half(renderer, mx - 34, my + 32, g.next_c1, .left)
	render_pill_half(renderer, mx - 12, my + 32, g.next_c2, .right)
}
