...
fn (sm &SoundManager) play_shoot_sound() {
	...
	for i in 0 .. num_samples {
		...
		freq := 1200.0 - (800.0 * (f64(i) / f64(num_samples)))
...
fn (sm &SoundManager) play_tractor_beam_sound() {
	...
	for i in 0 .. num_samples {
		t := f64(i) / f64(sample_rate)
		freq := 400.0 + 100.0 * math.sin(t * 40.0)
...
fn (sm &SoundManager) play_hit_sound() {
	...
	for i in 0 .. num_samples {
		t := f64(i) / f64(sample_rate)
		freq := 700.0 - 300.0 * (f64(i) / f64(num_samples))
		sine := math.sin(2.0 * math.pi * freq * t)
		env := math.exp(-30.0 * t)
		pcm[i] = i16(sine * env * 15000.0)
	}
	sdl.queue_audio(sm.dev, pcm.data, u32(num_samples * 2))
}
