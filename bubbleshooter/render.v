...
pub fn draw_glossy_bubble(renderer &sdl.Renderer, cx f64, cy f64, rad f64, color_id int) {
...
pub fn render_bubbleshooter_game(renderer &sdl.Renderer, game &BubbleShooterGame, win_w int, win_h int, particles []PopParticle) {
	...
	for r in 0 .. grid_rows {
		...
		for c in 0 .. max_c {
			...
			if col > 0 {
				...
				draw_glossy_bubble(renderer, bx, by, bubble_radius, col)
			}
		}
	}
	...
	for fb in game.falling {
		draw_glossy_bubble(renderer, fb.x, fb.y, bubble_radius, fb.color)
	}
	...
	if game.projectile.active {
		draw_glossy_bubble(renderer, game.projectile.x, game.projectile.y, bubble_radius, game.projectile.color)
	}
	...
	if game.state == .aiming {
		draw_glossy_bubble(renderer, launcher_cx, launcher_cy, bubble_radius, game.current_color)
	}

	// Draw Next Bubble in Reserve Queue (Left Panel)
	draw_text(renderer, ax - 130, ay + 380, 'NEXT', 2, Color{r: 160, g: 180, b: 220})
	draw_glossy_bubble(renderer, f64(ax - 90), f64(ay + 430), bubble_radius * 0.9, game.next_color)
