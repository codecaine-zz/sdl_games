...
fn (mut g FroggerGame) init_objects() {
	...
	// Row 4: Turtles (right to left)
	for i in 0 .. 4 {
		g.objects << LaneObject{ obj_type: .turtles, row: 4, x: f32(i) * 220.0, width: 75.0, speed: -120.0 * speed_mult }
	}
	// Row 3: Large Logs (left to right)
	for i in 0 .. 2 {
		g.objects << LaneObject{ obj_type: .log_large, row: 3, x: f32(i) * 420.0, width: 170.0, speed: 150.0 * speed_mult }
	}
...
fn (mut g FroggerGame) update(dt f32) {
	...
	for mut obj in g.objects {
		obj.x += obj.speed * dt
		if obj.obj_type == .turtles && obj.row == 1 {
