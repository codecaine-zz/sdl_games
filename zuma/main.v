...
fn draw_marble(renderer &sdl.Renderer, cx int, cy int, color_id int, power PowerupType) {
	if color_id == 0 {
		return
	}
	r := render_ball_r
...
fn (mut app App) render() {
	...
	for ball in app.game.balls {
		...
		draw_marble(renderer, int(pos_x) + sx, int(pos_y) + sy, ball.color, ball.powerup)
	}

	// Render Flying Projectile
	if app.game.projectile.active {
		draw_marble(renderer, int(app.game.projectile.x) + sx, int(app.game.projectile.y) + sy, app.game.projectile.color, .none)
	}
	...
	draw_marble(renderer, turret_cx, turret_cy, app.game.current_ball, .none)
	...
	draw_marble(renderer, back_bx, back_by, app.game.next_ball, .none)
	...
	if app.game.state == .level_won {
		...
		draw_text_centered(renderer, win_width / 2, win_height / 2 + 15, 'PRESS [R] TO RESTART', 2, Color{r: 255, g: 255, b: 255})
	} else if app.paused {
