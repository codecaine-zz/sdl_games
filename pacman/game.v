...
fn (mut g Game) update_pacman(dt f64) (bool, bool) {
	...
	// Always allow movement — wall-stopping only applies when truly blocked
	// Use grid check against the DESTINATION tile, not current tile
	...
	if !blocked {
		g.pacman.x += f64(dc2) * g.pacman.speed * dt
		g.pacman.y += f64(dr2) * g.pacman.speed * dt
		...
		// Snap to center of current tile when hitting a wall (regardless of
		// approach side, otherwise Pac-Man can lock up off-center forever)
		if dc2 != 0 {
			g.pacman.x = cx
		} else if dr2 != 0 {
			g.pacman.y = cy
...
fn (mut g Game) update(dt f64) (bool, bool, bool, bool, bool) {
	mut ate_dot := false
	mut ate_power := false
	mut ate_ghost := false
	mut pac_died := false
	...
	match g.status {
		...
		.playing {
			...
			ate_dot, ate_power = g.update_pacman(dt)
			ate_ghost, pac_died = g.update_ghosts(dt)
	...
	return ate_dot, ate_power, ate_ghost, pac_died, level_win
}
