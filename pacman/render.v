...
fn draw_pacman_sprite(renderer &sdl.Renderer, px f64, py f64, dir Direction, mouth_ang f64) {
	cx := margin_x + int(px)
	cy := margin_y + int(py)
	r := 11
...
fn draw_ghost_sprite(renderer &sdl.Renderer, ghost &Ghost, global_t f64, frightened_t f64) {
...
fn draw_game(renderer &sdl.Renderer, game &Game, mouse_x int, mouse_y int, btn_reset &Button, btn_sound &Button, btn_pause &Button) {
	...
	for l in 0 .. game.lives {
		draw_pacman_sprite(renderer, f64(750 + l * 26), -50.0, .right, 0.3)
	}
	...
	if game.fruit_active {
		fx := margin_x + game.fruit_col * cell_px + 6
		fy := margin_y + game.fruit_row * cell_px + 6
		f_rect := sdl.Rect{x: fx, y: fy, w: 12, h: 12}
	...
	for g in game.ghosts {
		draw_ghost_sprite(renderer, &g, game.global_timer, game.frightened_timer)
	}

	// Render Pac-Man
	draw_pacman_sprite(renderer, game.pacman.x, game.pacman.y, game.pacman.dir, game.pacman.mouth_angle)
