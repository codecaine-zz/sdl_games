...
pub fn render_yie_ar_kung_fu_game(renderer &sdl.Renderer, mut g YieArKungFuGame) {
	...
	render_player_oolong(renderer, mut g, shake_x, shake_y)
	render_opponent_master(renderer, mut g, shake_x, shake_y)
...
fn render_player_oolong(renderer &sdl.Renderer, mut g YieArKungFuGame, ox int, oy int) {
	p := g.player
	px := int(p.x) + ox
	py := int(p.y) + oy
	ph := int(p.height)
...
fn render_opponent_master(renderer &sdl.Renderer, mut g YieArKungFuGame, ox_offset int, oy_offset int) {
	op := g.opponent
