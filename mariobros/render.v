...
fn render_ambient_torches(renderer &sdl.Renderer, ox int, oy int) {
	...
	for t in torches {
		...
		sdl.set_render_draw_color(renderer, 255, 140, 20, u8(18 + flicker * 2))
		glow_rect := sdl.Rect{ x: tx - 28, y: ty - 28, w: 56, h: 56 }
		sdl.render_fill_rect(renderer, &glow_rect)

		sdl.set_render_draw_color(renderer, 255, 200, 40, u8(35 + flicker * 3))
		inner_glow := sdl.Rect{ x: tx - 14, y: ty - 14, w: 28, h: 28 }
		...
		sdl.set_render_draw_color(renderer, 40, 40, 50, 255)
		bracket := sdl.Rect{ x: tx - 6, y: ty + 12, w: 12, h: 3 }
		sdl.render_fill_rect(renderer, &bracket)

		// Dancing Torch Flame (Yellow / Orange / Red)
		flame_h := 8 + flicker
...
fn render_steam_and_mist(renderer &sdl.Renderer, ox int, oy int) {
	...
	for sp in pipe_steam_spots {
		...
		for puff := 0; puff < 3; puff++ {
			phase := f64((int(ticks) + puff * 400) % 1800) / 1800.0
			puff_y := sy - int(phase * 45.0)
			puff_x := sx + int(math.sin(phase * 4.0) * 8.0)
			size := int(8.0 + phase * 18.0)
			alpha := u8((1.0 - phase) * 45.0)

			sdl.set_render_draw_color(renderer, 200, 230, 255, alpha)
...
fn render_puddle_reflections(renderer &sdl.Renderer, ox int, oy int) {
	...
	for i := 0; i < 4; i++ {
		px := 180 + i * 140 + ox
		py := 544 + oy
		ripple := int(math.sin(f64(ticks) / 220.0 + f64(i)) * 3.0)

		sdl.set_render_draw_color(renderer, 40, 140, 220, 50)
		puddle := sdl.Rect{ x: px - 40, y: py, w: 80, h: 4 + ripple }
		sdl.render_fill_rect(renderer, &puddle)

		// Specular light sheen
		sdl.set_render_draw_color(renderer, 180, 230, 255, 70)
		sheen := sdl.Rect{ x: px - 20, y: py + 1, w: 40, h: 1 }
