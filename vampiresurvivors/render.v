...
pub fn (g &Game) render_world(renderer &sdl.Renderer) {
	...
	for r := start_row; r <= end_row; r++ {
		for c := start_col; c <= end_col; c++ {
			...
			if (r * 13 + c * 7) % 19 == 0 {
				draw_tombstone(renderer, sx + 24, sy + 18)
			}
		}
	}
	...
	for br in g.breakables {
		bx := int(br.x) - cam_x
		by := int(br.y) - cam_y
		if bx < -30 || bx > win_width + 30 || by < -30 || by > win_height + 30 {
			continue
		}
		if br.is_urn {
			draw_urn(renderer, bx, by)
		} else {
			draw_candelabra(renderer, bx, by)
		}
	}
	...
	for item in g.floor_pickups {
		ix := int(item.x) - cam_x
		iy := int(item.y) - cam_y
		if ix < -30 || ix > win_width + 30 || iy < -30 || iy > win_height + 30 {
			continue
		}
		draw_floor_pickup(renderer, ix, iy, item.kind)
	}
	...
	for gem in g.gems {
		gx := int(gem.x) - cam_x
		gy := int(gem.y) - cam_y
		if gx < -20 || gx > win_width + 20 || gy < -20 || gy > win_height + 20 {
			continue
		}
		draw_exp_gem(renderer, gx, gy, gem.kind)
	}
	...
	for e in g.enemies {
		ex := int(e.x) - cam_x
		ey := int(e.y) - cam_y
		if ex < -60 || ex > win_width + 60 || ey < -60 || ey > win_height + 60 {
			continue
		}
		draw_enemy_sprite(renderer, ex, ey, e, g.frozen_timer > 0)
	}
	...
	for pr in g.projectiles {
		px := int(pr.x) - cam_x
		py := int(pr.y) - cam_y
		draw_projectile(renderer, px, py, pr)
	}
	...
	for ep in g.enemy_projectiles {
		epx := int(ep.x) - cam_x
		epy := int(ep.y) - cam_y
		if epx < -20 || epx > win_width + 20 || epy < -20 || epy > win_height + 20 {
			continue
		}
		rad := int(ep.radius)
	...
	for p in g.players {
		...
		px := int(p.x) - cam_x
		py := int(p.y) - cam_y
		draw_player_sprite(renderer, px, py, p)
	}
...
fn draw_tombstone(renderer &sdl.Renderer, x int, y int) {
	sdl.set_render_draw_color(renderer, 50, 58, 75, 255)
	t_rect := sdl.Rect{x: x, y: y + 4, w: 16, h: 20}
	...
	sdl.render_draw_line(renderer, x + 8, y + 7, x + 8, y + 18)
	sdl.render_draw_line(renderer, x + 4, y + 11, x + 12, y + 11)
}

fn draw_candelabra(renderer &sdl.Renderer, x int, y int) {
...
fn draw_urn(renderer &sdl.Renderer, x int, y int) {
...
fn draw_floor_pickup(renderer &sdl.Renderer, x int, y int, kind FloorPickupType) {
	ticks := sdl.get_ticks()
...
fn draw_exp_gem(renderer &sdl.Renderer, x int, y int, kind GemType) {
	match kind {
		.blue {
...
fn draw_player_sprite(renderer &sdl.Renderer, x int, y int, p Player) {
	// Shadow
...
fn draw_enemy_sprite(renderer &sdl.Renderer, x int, y int, e Enemy, is_frozen bool) {
	if e.is_champion {
		sdl.set_render_draw_color(renderer, 255, 215, 60, 255)
		rad := int(e.radius) + 3
...
fn draw_projectile(renderer &sdl.Renderer, x int, y int, pr Projectile) {
	match pr.kind {
		.whip {
		...
		.cataclysm_nuke {
			// Massive cluster bomb explosion
			rad := int(pr.radius)
			sdl.set_render_draw_blend_mode(renderer, .blend)
		...
		.prismatic_laser {
			// Wide screen-penetrating prismatic beam
...
pub fn (g &Game) render_hud(renderer &sdl.Renderer) {
	...
	mut icon_x := 20
	for w in p1.weapons {
		w_box := sdl.Rect{x: icon_x, y: 28, w: 28, h: 28}
		...
		draw_text(renderer, icon_x + 18, 44, '${w.level}', 1, Color{r: 100, g: 240, b: 255})
		icon_x += 34
	}

	mut p_icon_x := 20
	for pass in p1.passives {
		p_box := sdl.Rect{x: p_icon_x, y: 62, w: 22, h: 22}
		...
		draw_text(renderer, p_icon_x + 6, 67, let, 1, Color{r: 120, g: 255, b: 150})
		draw_text(renderer, p_icon_x + 13, 73, '${pass.level}', 1, Color{r: 255, g: 255, b: 255})
		p_icon_x += 26
	}
...
pub fn (g &Game) render_level_up_modal(renderer &sdl.Renderer) {
	...
	for i, card in g.upgrade_cards {
		...
		draw_text(renderer, card_x + 16, card_y + 14, '[${i + 1}] ${card.name} (LV ${card.level})', 2, Color{r: 255, g: 255, b: 255})
		draw_text(renderer, card_x + card_w - 140, card_y + 14, tag_txt, 1, tag_col)
		draw_text(renderer, card_x + 16, card_y + 44, card.desc, 1, Color{r: 190, g: 205, b: 235})
	}
}
...
pub fn (g &Game) render_chest_modal(renderer &sdl.Renderer) {
	...
	draw_exp_gem(renderer, win_width / 2, win_height / 2 - 50, .chest)
...
pub fn (g &Game) render_character_select(renderer &sdl.Renderer) {
	...
	chars := [
		['ANTONIO BELPAESE', 'WEAPON: WHIP', '+10% MELEE DAMAGE', 'ULT: BLOOD TEMPEST'],
		...
		['GENNARO BELPAESE', 'WEAPON: KNIFE', '+1 EXTRA PROJECTILE', 'ULT: BLADE HURRICANE'],
	]
	...
	for i, c_info in chars {
		...
		draw_player_sprite(renderer, card_x + card_w / 2, card_y + 65, dummy_p)

		draw_text_centered(renderer, card_x + card_w / 2, card_y + 120, '[${i + 1}]', 2, Color{r: 255, g: 215, b: 60})
		draw_text_centered(renderer, card_x + card_w / 2, card_y + 150, c_info[0], 1, Color{r: 255, g: 255, b: 255})
		draw_text_centered(renderer, card_x + card_w / 2, card_y + 180, c_info[1], 1, Color{r: 100, g: 220, b: 255})
		draw_text_centered(renderer, card_x + card_w / 2, card_y + 210, c_info[2], 1, Color{r: 120, g: 255, b: 150})
		draw_text_centered(renderer, card_x + card_w / 2, card_y + 240, c_info[3], 1, Color{r: 255, g: 140, b: 220})
	}
