...
fn (mut g BreakoutGame) step(dt f64, move_x f64, launch_trigger bool, fire_trigger bool) {
	...
	for i := g.capsules.len - 1; i >= 0; i-- {
		...
		// Catch check
		if cap.y + f64(cap.h) >= g.paddle.y && cap.y <= g.paddle.y + g.paddle.h {
			if cap.x + f64(cap.w) >= g.paddle.x && cap.x <= g.paddle.x + g.paddle.w {
	...
	for i := g.balls.len - 1; i >= 0; i-- {
		...
		if ball.attached {
			...
			ball.y = g.paddle.y - ball.radius - 2.0
			g.balls[i] = ball
			continue
		}
		...
		// Paddle Bounce
		if ball.dy > 0 && ball.y + ball.radius >= g.paddle.y && ball.y - ball.radius <= g.paddle.y + g.paddle.h {
			if ball.x >= g.paddle.x - 4.0 && ball.x <= g.paddle.x + g.paddle.w + 4.0 {
				g.last_sound_event = 'paddle'
				g.combo_multiplier = 1
				...
				if g.paddle.has_sticky && g.paddle.sticky_catches > 0 {
					...
					ball.attached = true
					ball.offset_x = ball.x - g.paddle.x
				} else {
					// Hit angle calculation relative to paddle center
					hit_pos := (ball.x - (g.paddle.x + g.paddle.w / 2.0)) / (g.paddle.w / 2.0)
					angle := hit_pos * (math.pi / 3.0) // Max 60 deg bounce angle
