...
pub fn render_yahtzee_game(renderer &sdl.Renderer, mut g YahtzeeGame, win_w int, win_h int, mouse_x int, mouse_y int, sound_enabled bool) {
	...
	tray_w := 460
	tray_h := 570
	render_dice_tray(renderer, g, tray_x, tray_y, tray_w, tray_h, mouse_x, mouse_y)
...
fn render_dice_tray(renderer &sdl.Renderer, g YahtzeeGame, x int, y int, w int, h int, mouse_x int, mouse_y int) {
	// Wood Tray Inset
	...
	cup_x := x + w / 2
	cup_y := y + 160
	render_shaker_cup(renderer, cup_x, cup_y, g.is_rolling())

	// Draw 5 Dice
	for i, d in g.dice {
		dx := int(d.x)
		dy := int(d.y)
		render_single_die(renderer, dx, dy, d.value, d.held, i + 1)
	}
...
fn render_shaker_cup(renderer &sdl.Renderer, cx int, cy int, is_shaking bool) {
	mut shake_ox := 0
	if is_shaking {
...
fn render_single_die(renderer &sdl.Renderer, x int, y int, value int, held bool, num int) {
	d_size := 64
	half := d_size / 2
