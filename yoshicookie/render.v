...
pub fn render_yoshi_cookie_game(renderer &sdl.Renderer, mut g YoshiCookieGame) {
	...
	// 3. 3D Baked Cookies
	render_cookies(renderer, mut g, shake_x, shake_y)
	...
	render_reserve_plate(renderer, mut g)

	// 5. Mario Chef & Yoshi Characters
	render_mario_chef(renderer, mut g)
	render_yoshi_bakery(renderer, mut g)
...
fn render_reserve_plate(renderer &sdl.Renderer, mut g YoshiCookieGame) {
	px := 42
	py := 395
	...
	if g.has_reserve && g.reserve_cookie != .none {
		render_single_cookie(renderer, px + 5, py + 3, g.reserve_cookie)
	}
}
...
fn render_cookies(renderer &sdl.Renderer, mut g YoshiCookieGame, ox int, oy int) {
	for r in 0 .. 8 {
		for c in 0 .. 8 {
			cookie := g.grid[r][c]
			if cookie == .none {
			...
			render_single_cookie(renderer, cx, cy, cookie)
		}
	}
}

fn render_single_cookie(renderer &sdl.Renderer, x int, y int, cookie CookieType) {
	// Cookie Base Shadow
	sdl.set_render_draw_blend_mode(renderer, .blend)
...
fn render_yoshi_bakery(renderer &sdl.Renderer, mut g YoshiCookieGame) {
	// Yoshi sitting on Right Side (x: 650, y: 190)
	yx := 650
	yy := 190

	// Yoshi Green Body
	sdl.set_render_draw_color(renderer, 50, 185, 60, 255)
	sdl.render_fill_rect(renderer, &sdl.Rect{ x: yx + 18, y: my_adj(yy, 40), w: 42, h: 56 })
