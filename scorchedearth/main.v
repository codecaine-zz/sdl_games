...
fn main() {
	...
	for running {
				...
				.mousebuttondown {
					...
					if event.button.button == u8(sdl.button_left) {
						...
						if app.game.in_shop {
							...
							mut row_y := 160
							for w in weps {
								// Click on entire row or buy button
							...
							if my >= win_h - 55 {
								if mx >= win_w - 220 {
									app.game.fire_shot()
								} else if mx >= 380 && mx <= win_w - 230 {
									// Cycle weapons
									mut cur_t := app.game.tanks[app.game.current_turn]
									cur_t.active_wep = match cur_t.active_wep {
										.standard { WeaponType.baby_nuke }
										.baby_nuke { WeaponType.mirv }
										.mirv { WeaponType.mountain_mover }
										.mountain_mover { WeaponType.napalm }
										.napalm { WeaponType.digger }
										.digger { WeaponType.standard }
									}
									app.game.tanks[app.game.current_turn] = cur_t
								}
							}
		...
		if app.game.sound_event != '' {
			...
			match ev {
				'shot' { app.sound_mgr.play_shot() }
				'exp' { app.sound_mgr.play_explosion(false) }
				'nuke' { app.sound_mgr.play_explosion(true) }
				'mirv' { app.sound_mgr.play_mirv_split() }
