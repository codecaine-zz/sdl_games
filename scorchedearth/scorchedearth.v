...
pub mut:
	...
	x            int
	y            int
	angle        f64 = 45.0 // Degrees
	power        f64 = 500.0 // 50..1000
	health       int = 100
	max_health   int = 100
	cash         int = 1000
	color        Color
	inventory    map[string]int // Weapon name -> count
...
pub mut:
	width           int = 920
	height          int = 640
	terrain_y       []int // height at each x: 0..width-1
	tanks           []Tank
	current_turn    int
	wind            f64 // -80.0 to +80.0
	round           int = 1
	max_rounds      int = 5
	in_shop         bool
	is_game_over    bool

	// Active projectiles, explosions, and floating damage texts
...
pub fn (mut g ScorchedGame) select_weapon(w WeaponType) {
	...
	g.banner_text = 'ARMED: ${info.name.to_upper()}'
	g.banner_timer = 1.5
}
...
pub fn (mut g ScorchedGame) fire_shot() bool {
	...
	// Consume inventory weapon if not standard
	if cur_tank.active_wep != .standard {
		w_name := cur_tank.active_wep.str()
	...
	start_y := f64(cur_tank.y - 8) - math.sin(rad) * barrel_len

	g.projectiles << Projectile{
		wtype: cur_tank.active_wep
		x: start_x
		y: start_y
		vx: math.cos(rad) * spd
		vy: -math.sin(rad) * spd
...
pub fn (mut g ScorchedGame) update(dt f64) {
	...
	for i in 0 .. g.projectiles.len {
		...
		// Check Direct Hit against any Tank
		mut hit_tank := false
		...
		if p.wtype == .mirv && !p.split_done && p.vy > 0.0 && p.y < 350.0 {
			p.split_done = true
			g.sound_event = 'mirv'
			for sub_i in -2 .. 3 {
				if sub_i != 0 {
					g.projectiles << Projectile{
						...
						x: p.x
						y: p.y
						vx: p.vx + f64(sub_i) * 55.0
						vy: p.vy + rand.f64() * 30.0
						owner_id: p.owner_id
						active: true
						split_done: true
					}
...
fn (mut g ScorchedGame) detonate_projectile(p Projectile) {
	...
	g.sound_event = if p.wtype == .baby_nuke { 'nuke' } else { 'exp' }
	...
	g.explosions << Explosion{
		...
		life: 0.0
		max_l: if p.wtype == .baby_nuke { 0.65 } else { 0.4 }
		is_nuke: p.wtype == .baby_nuke
		col: if p.wtype == .baby_nuke { Color{255, 200, 50, 255} } else { Color{255, 100, 30, 255} }
	}

	// Modify Terrain
	if p.wtype == .mountain_mover {
		// Dirt bomb: raises terrain
	...
	for ti := 0; ti < g.tanks.len; ti++ {
		...
		if dist <= max_dist {
			...
			// Direct hits (< 18px) deal 100% full damage
			dmg := if dist <= 18.0 { info.damage } else { int(f64(info.damage) * factor) }
...
fn (mut g ScorchedGame) advance_turn() {
	...
	if alive_count <= 1 {
		// Delay shop opening by 2.0s so death explosion and banner are fully seen
		if g.round_end_timer <= 0.0 {
