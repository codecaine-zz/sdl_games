...
fn render_platform(renderer &sdl.Renderer, p Platform, ticks u32) {
	if p.broken {
		return
	}
	px := int(p.x)
	...
	match p.kind {
		.standard {
			// Steel Blue with Neon Cyan Glow
		...
		.ice {
			...
			// Ice Crystal Shimmer
			for ix := px + 8; ix < px + pw - 8; ix += 20 {
				sdl.set_render_draw_color(renderer, 255, 255, 255, 200)
				sdl.render_draw_line(renderer, ix, py + 3, ix + 4, py + ph - 3)
			}
		}
		.spring {
			// Golden Amber Spring Platform with Bouncing Coil Loops
			sdl.set_render_draw_color(renderer, 245, 158, 11, 255)
			sdl.render_fill_rect(renderer, &rect)
			sdl.set_render_draw_color(renderer, 254, 240, 138, 255)
			sdl.render_draw_rect(renderer, &rect)

			// Spring coil loops underneath
			coil_y := py + ph
			for cx := px + 10; cx < px + pw - 10; cx += 16 {
				coil_offset := int(math.sin(f64(ticks) * 0.02 + f64(cx)) * 2.0)
				coil_rect := sdl.Rect{
					x: cx
					y: coil_y + coil_offset
					w: 10
					h: 6
				}
				sdl.set_render_draw_color(renderer, 250, 204, 21, 255)
				sdl.render_draw_rect(renderer, &coil_rect)
			}
		}
		.crumbly {
			// Fiery Crimson Red Crumbly Stone with Pulsing Warning Glow
			...
			// Crackle stone pattern
			for cx := px + 12; cx < px + pw - 12; cx += 18 {
				sdl.set_render_draw_color(renderer, 153, 27, 27, 255)
				sdl.render_draw_line(renderer, cx, py + 2, cx - 4, py + ph - 2)
			}
		}
	}
}

fn render_jumper(renderer &sdl.Renderer, j Jumper) {
	if !j.is_alive {
		return
	}
	jx := int(j.x)
	...
	sdl.render_fill_rect(renderer, &scarf_rect)
	...
	// Feet with foot-tapping idle animation from xjump-sdl
	foot_y := jy + jh - 4
	foot_off := if j.on_ground && j.is_idle_variant { 2 } else { 0 }
	foot1 := sdl.Rect{
		x: jx + 4 + foot_off
		y: foot_y
		w: 10
...
fn render_lava(renderer &sdl.Renderer, lava_y f64, ticks u32) {
	ly := int(lava_y)
	if ly >= win_height {
		return
	}
	...
	// Animated Undulating Heat Wave Crests
	sdl.set_render_draw_color(renderer, 250, 204, 21, 255)
	for x := wall_w; x < win_width - wall_w; x += 12 {
