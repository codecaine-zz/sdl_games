...
fn (mut app App) init_sdl() bool {
	...
	app.window = sdl.create_window(c'GNUjump - V & SDL2', sdl.windowpos_centered, sdl.windowpos_centered,
		win_width, win_height, u32(sdl.WindowFlags.shown))
...
fn (mut app App) render() {
	...
	if app.game.mode == .playing {
		...
		for p in app.game.platforms {
			render_platform(app.renderer, p, ticks)
		}

		// Render Lava Surface
		render_lava(app.renderer, app.game.lava_y, ticks)

		// Render Jumpers
		render_jumper(app.renderer, app.game.p1)
		if app.game.game_mode == .pvp {
			render_jumper(app.renderer, app.game.p2)
		}
	}
