	...
	match g.player.state {
		.walking {
			...
			if !on_solid_floor {
				...
				for i, tr in g.trampolines {
					if math.abs(g.player.x - tr.x) < tr.width * 0.75 {
						if !tr.is_broken {
							g.player.state = .bouncing
							g.player.trampoline_idx = i
							g.player.x = tr.x
							// Calculate bounce phase matching entry Y position
		...
		.bouncing {
			...
			if g.player.bounce_phase >= 2.0 * math.pi {
				...
				// Reached bottom of trampoline: increment trampoline wear!
			...
			// Y position along sinusoidal bounce: peak at y=90, bottom at y=500
			min_y := 90.0
			...
			if left || right {
				...
				for fl_idx in 0 .. 6 {
					...
					if math.abs(g.player.y - target_y) < 28.0 {
