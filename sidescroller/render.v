...
fn draw_game(renderer &sdl.Renderer, ge &GameEngine) {
	...
	for pu in ge.powerups {
		draw_powerup(renderer, pu, ge.camera_x, shake_x, shake_y)
	}

	// 5. Enemies & Bosses
	for enemy in ge.enemies {
		if enemy.active {
			draw_enemy(renderer, enemy, ge.camera_x, shake_x, shake_y)
		}
	}

	// 6. Player
	if ge.state == .playing || ge.state == .stage_clear || ge.state == .paused {
		draw_player(renderer, ge.player, ge.camera_x, shake_x, shake_y, ge.elapsed_time)
	}
...
fn draw_player(renderer &sdl.Renderer, p Player, camera_x f64, sx int, sy int, t f64) {
...
fn draw_enemy(renderer &sdl.Renderer, enemy Enemy, camera_x f64, sx int, sy int) {
...
fn draw_powerup(renderer &sdl.Renderer, pu PowerUp, camera_x f64, sx int, sy int) {
