...
fn draw_panel(renderer &sdl.Renderer, cx int, cy int, size int, panel_type int) {
	if panel_type == 0 {
		return
	}
	r := size / 2 - 2
...
fn (mut app App) render() {
	...
	for r in 0 .. pdp_rows {
		for c in 0 .. pdp_cols {
			...
			if p_val != 0 {
				...
				if cy >= by {
					draw_panel(renderer, cx, cy, cell_size, p_val)
				}
			}
		}
	}
	...
	for c in 0 .. pdp_cols {
		...
		draw_panel(renderer, cx, cy, cell_size, app.game.next_row[c])
	}
	...
	sdl.render_draw_line(renderer, c_rect.x + c_rect.w, c_rect.y + c_rect.h, c_rect.x + c_rect.w, c_rect.y + c_rect.h - corner_len)
