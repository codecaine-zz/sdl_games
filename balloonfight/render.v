...
fn draw_game(renderer &sdl.Renderer, ge &GameEngine) {
	...
	for spark in ge.sparks {
		draw_spark(renderer, spark, ge.trip_scroll_x, ge.mode == .balloon_trip)
	}
	...
	if ge.mode == .balloon_trip {
		for tb in ge.trip_balloons {
			if !tb.collected {
				draw_trip_balloon(renderer, tb, ge.trip_scroll_x)
			}
		}
	}
	...
	if ge.state == .playing || ge.state == .phase_clear || ge.state == .paused {
		for p in ge.players {
			if p.state != .dead {
				draw_player(renderer, p, ge.elapsed_time)
			}
		}
	}
	...
	// Giant Fish
	if ge.fish.active {
		draw_giant_fish(renderer, ge.fish)
	}
...
fn draw_player(renderer &sdl.Renderer, p Player, t f64) {
	px := int(p.motion.x)
	py := int(p.motion.y)
	...
	wing_y := if int(t * 15.0) % 2 == 0 { py - 4 } else { py + 4 }
	sdl.render_draw_line(renderer, px - 10, py, px - 18, wing_y)
	sdl.render_draw_line(renderer, px + 10, py, px + 18, wing_y)

	// Draw Balloons or Parachute
	if p.state == .parachuting {
		...
		// Helium Balloons (1 or 2)
		if p.balloons >= 1 {
			draw_balloon(renderer, px - 10, py - 26, Color{
				r: 239
				g: 68
				b: 68
			})
...
fn draw_spark(renderer &sdl.Renderer, spark Spark, scroll_x f64, is_trip bool) {
	mut sx := int(spark.x)
	if is_trip {
...
fn draw_trip_balloon(renderer &sdl.Renderer, tb TripBalloon, scroll_x f64) {
	sx := int(tb.x - scroll_x)
	sy := int(tb.y)

	draw_balloon(renderer, sx, sy, Color{
		r: 168
		g: 85
		b: 247
	})
