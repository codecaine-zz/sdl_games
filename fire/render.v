...
pub fn draw_fire_game(renderer &sdl.Renderer, game &FireGame) {
	...
	draw_text(renderer, 620, 42, 'WIDE SCREEN', 2, Color{ r: 80, g: 70, b: 50 })
	...
	for wy in 0 .. 4 {
		...
		// Flames licking out of upper windows
		if wy < 2 {
			fill_rect(renderer, 105, 132 + wy * 60, 26, 12, g_accent_red)
		}
	}
	// Roof Fire
	fill_rect(renderer, 70, 100, 65, 20, g_accent_red)
	draw_text(renderer, 72, 104, 'FLAME', 1, g_plate_gold)

	// Static Artwork: Ambulance on the Right
	amb_x := 680
	amb_y := 330
	fill_rect(renderer, amb_x, amb_y, 60, 70, Color{ r: 240, g: 240, b: 240 })
	draw_rect(renderer, amb_x, amb_y, 60, 70, g_lcd_ink)
	fill_rect(renderer, amb_x + 10, amb_y + 10, 30, 20, g_accent_blue) // Window
	fill_rect(renderer, amb_x + 20, amb_y + 40, 20, 6, g_accent_red) // Red Cross horizontal
	fill_rect(renderer, amb_x + 27, amb_y + 33, 6, 20, g_accent_red) // Red Cross vertical
	fill_rect(renderer, amb_x + 22, amb_y - 8, 16, 8, g_accent_red) // Siren
	...
	draw_text(renderer, 240, 92, 'SCORE:', 2, g_lcd_ink)
	draw_text(renderer, 340, 88, '${game.score:04d}', 3, g_lcd_ink)
	draw_text(renderer, 620, 95, 'HI:${game.high_score}', 1, g_lcd_ink)
	draw_text(renderer, 150, 95, if game.mode == .game_a { 'GAME A' } else { 'GAME B' }, 1, g_lcd_ink)
	...
	draw_text_centered(renderer, 400, 530, 'Direct: [Z] Left  [X] Mid  [C] Right', 1, g_lcd_ink)
