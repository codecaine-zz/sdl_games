...
pub fn (g &DukeGame) is_solid(x f64, y f64) bool {
	...
	return tile == 1 || tile == 5 || tile == 6 || tile == 7
}
...
pub fn (mut g DukeGame) update(dt f64, move_dir int, climb_dir int, crouch bool, aim_up bool) DukeEvents {
	...
	// Check Ladder / Overhead Pipe Collision
	center_tx := int((g.player.x + g.player.w / 2.0) / tile_sz)
	center_ty := int((g.player.y + g.player.h / 2.0) / tile_sz)
	feet_ty := int((g.player.y + g.player.h) / tile_sz)
	top_ty := int(g.player.y / tile_sz)

	cur_tile := g.get_tile(center_tx, center_ty)
	feet_tile := g.get_tile(center_tx, feet_ty)
	top_tile := g.get_tile(center_tx, top_ty)

	is_on_ladder := cur_tile == 3 || feet_tile == 3 || top_tile == 3

	if is_on_ladder {
		if climb_dir < 0 { // Climbing UP
			// Check if we are at/near the top of the ladder
			top_ladder_y := f64(feet_ty * int(tile_sz)) - g.player.h
			if top_tile != 3 && cur_tile != 3 && feet_tile == 3 && g.player.y <= top_ladder_y + 4.0 {
				// Reached top platform! Dismount standing on top
				g.player.y = top_ladder_y
				g.player.vy = 0.0
		...
		} else if climb_dir > 0 { // Climbing DOWN
			g.player.is_climbing = true
			g.player.vy = 130.0
			g.player.on_ground = false
	...
	new_px := g.player.x + g.player.vx * dt
	if g.player.vx < 0.0 {
		if !g.is_solid(new_px, g.player.y + 2.0) &&
		   !g.is_solid(new_px, g.player.y + g.player.h / 2.0) &&
		   !g.is_solid(new_px, g.player.y + g.player.h - 2.0) {
			g.player.x = new_px
		}
	} else if g.player.vx > 0.0 {
		if !g.is_solid(new_px + g.player.w, g.player.y + 2.0) &&
		   !g.is_solid(new_px + g.player.w, g.player.y + g.player.h / 2.0) &&
		   !g.is_solid(new_px + g.player.w, g.player.y + g.player.h - 2.0) {
			g.player.x = new_px
		}
	}
	...
	front_tx := int((g.player.x + if g.player.dir > 0 { g.player.w + 6.0 } else { -6.0 }) / tile_sz)
	door_ty := int((g.player.y + g.player.h / 2.0) / tile_sz)
	front_tile := g.get_tile(front_tx, door_ty)

	if front_tile == 5 && g.player.has_red_key {
		g.tiles[door_ty][front_tx] = 0
		ev.unlocked_door = true
		g.player.score += 500
	} else if front_tile == 6 && g.player.has_blue_key {
		g.tiles[door_ty][front_tx] = 0
		ev.unlocked_door = true
		g.player.score += 500
	} else if front_tile == 7 && g.player.has_green_key {
		g.tiles[door_ty][front_tx] = 0
		ev.unlocked_door = true
		g.player.score += 500
	}
	...
	new_py := g.player.y + g.player.vy * dt
	g.player.on_ground = false

	if g.player.vy > 0.0 { // Falling down
		target_feet_ty := int((new_py + g.player.h) / tile_sz)
		is_ladder_top := g.get_tile(center_tx, target_feet_ty) == 3 && g.get_tile(center_tx, target_feet_ty - 1) != 3
		ladder_land := is_ladder_top && g.player.y + g.player.h <= f64(target_feet_ty * int(tile_sz)) + 6.0 && climb_dir <= 0

		if g.is_solid(g.player.x + 2.0, new_py + g.player.h) || g.is_solid(g.player.x + g.player.w - 2.0, new_py + g.player.h) || ladder_land {
			g.player.y = f64(target_feet_ty * int(tile_sz)) - g.player.h
			g.player.vy = 0.0
			g.player.on_ground = true
			g.player.is_climbing = false
		} else {
			// Check Elevator platform landing
	...
	} else if g.player.vy < 0.0 { // Jumping / Climbing up
		if g.is_solid(g.player.x + 2.0, new_py) || g.is_solid(g.player.x + g.player.w - 2.0, new_py) {
			g.player.y = f64((int(new_py / tile_sz) + 1) * int(tile_sz))
			g.player.vy = 0.0
		} else {
			g.player.y = new_py
		}
