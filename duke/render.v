...
pub fn update_sparks(mut sparks []SparkParticle, dt f64) {
	for mut p in sparks {
		p.x += p.vx * dt
		p.y += p.vy * dt + 60.0 * dt // gravity pull on debris
		p.life -= dt
	}
...
pub fn draw_duke_player(renderer &sdl.Renderer, p &DukePlayer, cx f64, cy f64) {
	ix := int(cx)
	iy := int(cy)
	d := p.dir

	// If somersaulting in air, render tumbling body
	if !p.on_ground && !p.is_climbing && !p.is_hanging {
		// Tumble ball
		sdl.set_render_draw_color(renderer, 220, 40, 40, 255) // Red Tank Top
	...
	// 4. Blue Jeans & Brown Boots (Walking cycle animation)
	if !p.is_crouching {
...
pub fn draw_tile(renderer &sdl.Renderer, tx int, ty int, tile int, sx int, sy int, level int) {
	rect := sdl.Rect{x: sx, y: sy, w: int(tile_sz), h: int(tile_sz)}

	match tile {
		1 { // Steel Wall / Floor themed by sector
			match level {
				2 { // Subterranean Rust / Amber
				...
				3 { // Orbital Cyber Tech (Purple/Cyan)
				...
				else { // Sector 1: Cyber City Night Steel
...
pub fn draw_boss_mech(renderer &sdl.Renderer, boss &BossMech, cam_x f64, cam_y f64) {
	if !boss.active { return }
	bx := int(boss.x - cam_x)
	by := int(boss.y - cam_y)

	// Heavy Quad-Armored Mech Walker Body
	...
	// Boss Health Bar directly above boss
	bar_w := 80
	bar_h := 8
	bar_x := bx - 5
	bar_y := by - 16
...
pub fn render_duke_game(renderer &sdl.Renderer, game &DukeGame, screen_w int, screen_h int, sparks []SparkParticle) {
	...
	match game.level_num {
		2 { sdl.set_render_draw_color(renderer, 24, 14, 12, 255) } // Rust Subterranean
		3 { sdl.set_render_draw_color(renderer, 18, 12, 32, 255) } // Cyber Orbital
		else { sdl.set_render_draw_color(renderer, 15, 18, 28, 255) } // Cityscape
	}
	sdl.render_clear(renderer)

	// Camera Scroll Offset (Center on Duke)
	...
	// Draw Parallax Distant Backdrop
	if game.level_num == 1 {
	...
	for r in start_row .. end_row {
		for c in start_col .. end_col {
			...
			if tile > 0 {
				...
				draw_tile(renderer, c, r, tile, sx, sy, game.level_num)
			}
		}
	}
	...
	// Draw Mega Mech Boss
	draw_boss_mech(renderer, &game.boss, cam_x, cam_y)
	...
	// Draw Duke Player
	draw_duke_player(renderer, &game.player, game.player.x - cam_x, game.player.y - cam_y)
